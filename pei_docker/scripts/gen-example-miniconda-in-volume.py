...
fn_output = f'pei_docker/examples/miniconda-in-volume.yml'
apt_repo : str = 'tuna'
...
cfg_obj.stage_2.custom = oc.OmegaConf.create({
    'on_first_run':['stage-2/system/conda/auto-install-miniconda.sh']
    })
...
for prefix, opt in s2_storage.items():
    if prefix == 'app':
        opt.type='manual-volume'
        opt.volume_name = f'my_{prefix}'
    else:
        opt.type='auto-volume'
