...
- [ ] 1.2 For each tool to migrate, identify any non-shell assets required at runtime (e.g., `litellm/proxy.py`, opengl JSON/YAML) and confirm they will be moved alongside scripts.
- [ ] 1.3 Identify scripts that reference `$PEI_STAGE_DIR_2` (especially `.../tmp/...`) and decide per-script whether to (a) switch to `$PEI_STAGE_DIR_1/tmp`, (b) add `--tmp-dir/--cache-dir` flags, or (c) keep stage-2-only glue in a wrapper.
...
- [ ] 3.2 For wrappers that must be sourced (if any), ensure they are source-safe (use `return` on error, with `exit` fallback when executed directly).
- [ ] 3.3 Update migrated installers that used `$PEI_STAGE_DIR_2/tmp` to use `$PEI_STAGE_DIR_1/tmp` instead; when writing, create the dir, and when reading-only, tolerate the dir missing (treat as cache miss).
...
- [ ] 4.2 Update docs/examples/templates referencing `stage-2/system/<tool>/...` paths to prefer `stage-1/system/<tool>/...` where appropriate (keeping stage-2 wrapper paths for backward compatibility).
