...
Most example directories below assume the default two-stage Compose workflow. If you want a `stage-1-only` container or the `merged build` workflow instead, start with [Build Modes](../docs/manual/getting-started/build-modes.md).

## Workflow Variants

- [Build Modes](../docs/manual/getting-started/build-modes.md) explains the three supported modes: `stage-1-only`, `two-stage Compose`, and `merged build`.
- For a runnable minimal `stage-1-only` variant and the merged-build walkthrough, use that page alongside [01 Minimal SSH](../docs/examples/basic/01-minimal-ssh.md).
...
## Default Two-Stage Usage Pattern
